...
from realestate_app.models import (
    State, County,
    Race, RaceEstimate, RaceStateTotal,
    Income, IncomeEstimate, IncomeStateTotal
)

from realestate_education.models import (
    Education, EducationEstimate, EducationStateTotal)

from realestate_poverty.models import (
    Poverty, PovertyEstimate, PovertyStateTotal)
...
class TopicTopState(APIView):
    ...
    def post(self, request):
        if request.data:
            ...
            print('race_data: ', type(race_data))

            income_data = topic_name[1]['Income']
            print('income_data: ', income_data)

            education_data = topic_name[2]['Education']
            print('education_data: ', education_data)

            poverty_data = topic_name[3]['Poverty']
            print('poverty_data: ****************', poverty_data)

            first_data = Race.objects.get(race_id=race_data)
            print('first_data: ------------->', type(first_data.race_id))
            second_data = Income.objects.get(income_id=income_data)
            print('second_data: -----------------> ', second_data)
            third_data = Education.objects.get(education_id=education_data)
            print('third_data: -------------->>', third_data)
            fourth_data = Poverty.objects.get(poverty_id=poverty_data)
            print('fourth_data: ----------->>>', fourth_data)
            ...
            if topic_name[0]['Race'] == first_data.race_id:
                Race_value = topic_name[0]['value']

                race_state_top = RaceStateTotal.objects.exclude(race_id='B02001001').annotate(
                    percent=F('race_total')*100/F('state_total')).filter(race_id=first_data)
            ...
            if topic_name[1]['Income'] == second_data.income_id:
            ...
            if topic_name[2]['Education'] == third_data.education_id:
                education_value = topic_name[2]['value']

                education_state_top = EducationStateTotal.objects.exclude(education_id='B19001001').annotate(
            ...
            if topic_name[3]['Poverty'] == fourth_data.poverty_id:
    ...
    def post(self, request):
        ...
        if request.data:
            ...
            if topic_name[2]['Education'] == third_data.education_id:
                ...
                education_county_top = EducationEstimate.objects.exclude(education_id='B19001001').annotate(
    ...
    def post(self, request):
        if request.data:
            ...
            if topic_name[2]['Education'] == third_data.education_id:
                ...
                education_state_county = EducationEstimate.objects.exclude(education_id='B19001001').annotate(
...
#                 top_race_state = RaceStateTotal.objects.exclude(race_id='B02001002').annotate(
#                     percent=F('race_total')*100/F('state_total'))
#                 print('top_race_state: ', top_race_state)
#             else:
#                 return Response({'error': "Invalid input value topic name RACE"})
